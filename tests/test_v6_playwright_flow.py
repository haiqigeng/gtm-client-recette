...
from core.report import compact_status_view, status_view
from core.state import StateError, read_stream
from core.workflow import complete_action, next_action
...
class PlaywrightDefaultFlowTests(unittest.TestCase):
    ...
    def test_fresh_context_requires_an_isolated_runtime_refresh(self) -> None:
        ...
        with self.assertRaisesRegex(StateError, "requires a verified isolated profile"):
            next_action(
                harness.run,
                {"capability": harness.capability(), "health": harness._health("before")},
                event_ids=["E-view_item"],
            ...
            completed = complete_action(
                harness.run,
                {
                    ...
                    "network": harness.network(
                        ["view_item"], action_id=action_id, payloads=[payload]
                    ),
    ...
    def test_complete_refuses_to_skip_a_required_preview_layer(self) -> None:
        harness = V5Harness(self.root)
        started = next_action(
            harness.run,
            {"capability": harness.capability(), "health": harness._health("before")},
        ...
        action_id = started["action"]["data"]["action_id"]
        with self.assertRaisesRegex(StateError, "requires one bounded Preview delta"):
            complete_action(
                harness.run,
                {
                    "binding": harness.binding(),
